...
\begin{prop}\proplabel{一様収束-一様収束の一様ノルムを用いた表現}
    ...
    \begin{proof}
        ...
        \begin{equation}
            \norm{f_n-f}<\varepsilon\ \Longleftrightarrow\ \sup_{x\in A}\bra{\norm{\fun{f_n}{x}-\fun{f}{x}}}<\varepsilon\ \Longleftrightarrow\ \forall x\in A,\, \norm{\fun{f_n}{x}-\fun{f}{x}}<\varepsilon
        \end{equation}
        である。
        ...
        \begin{align}
            ...
            \forall n\in \N,\, \bsqu{N\leq n\ \Longrightarrow\ \norm{\fun{f_n}{x}-\fun{f}{x}}<\varepsilon}
        \end{align}
        となり，\defiref{一様収束-各点収束と一様収束の定義}の\equref{一様収束-一様収束の定義}と同値であることが示された。
    \end{proof}
\end{prop}
...
\begin{thm}[一様収束性とCauchy列の同値性]\thmlabel{一様収束-一様収束性とCauchy列の同値性}
    ...
    \begin{proof}
        ...
        \end{equation}
        が成立している。
        更に，\lemref{実ノルム-ベクトルと関数のノルムの不等式}から，
        \begin{equation}
