...
\begin{defi}[ノルム]\defilabel{ノルム-定義}
...
\begin{prop}[三角不等式]\proplabel{ノルム-三角不等式}
    ノルムは次の性質も満たす：
    \begin{equation}
...
\begin{cor}\corlabel{ノルム-三角不等式}
    \propref{ノルム-三角不等式}において，$y\mapsto -y$として，元の三角不等式と併せれば，
    \begin{equation}
        \abs{\norm{x}-\norm{y}}\leq \norm{x+y}\leq \norm{x}+\norm{y}
    \end{equation}
    が従う。
...
\begin{prop}[ノルムの連続性]\proplabel{ノルム-連続性}
    ノルムは連続関数である。
    \begin{proof}
        示すべきことは以下の通り（\defiref{ノルム-関数の連続性}も参照）：
        \begin{equation}
        ...
        実際，$\delta\coloneqq \varepsilon$と取れば，\propref{ノルム-三角不等式}により，$\abs{\norm{x}-\norm{y}}\leq \norm{x-y}<\delta =\varepsilon$であるから，ノルムは連続関数である。
    \end{proof}
\end{prop}

\begin{defi}[ノルムの同値性]\defilabel{ノルム-ノルムの同値性}
...
\begin{thm}\thmlabel{ノルム-同値性}
    ...
    \begin{proof}
        ...
        \end{enumerate}
        以上により\equref{ノルム-max}で定められた$\norm{\dummy}$はノルムである。

        次に，$V$上の勝手なノルム$\norm{\dummy}_1$を取ってきたときに，\equref{ノルム-max}で定義した$\norm{\dummy}_1$と$\norm{\dummy}$が同値になってしまうことを示す。
        $V$のコンパクト集合$S$を$S\coloneqq \sset{y\in V}{\norm{y}=1}$
        によって定め，関数$f\colon S\to \R$を$\fun{f}{y}\coloneqq \norm{y}_1$と定める。
        $f$の連続性（\propref{ノルム-連続性}）と$S$がコンパクト集合であることから，$f$の値域には最小元$m$と最大元$M$が存在する。
        ...
        有限次元線型空間$V$上で与えられた任意の2つのノルム$\norm{\dummy}_1$, $\norm{\dummy}_2$はそれぞれ\equref{ノルム-max}で定義された$\norm{\dummy}$と同値であり，ノルムの同値は同値関係（\propref{ノルム-同値関係}）なので，$\norm{\dummy}_1$と$\norm{\dummy}_2$も同値である。
    \end{proof}
\end{thm}


\subsection{実線型空間上のノルム}\subseclabel{ノルム-実線型空間上のノルム}
...
\begin{eg}[Euclidノルム]\eglabel{ノルム-Euclidノルム}
    原点からの標準的な距離：
    \begin{equation}
...
\begin{eg}[一様ノルム]\eglabel{ノルム-一様}
    \thmref{ノルム-同値性}の証明において\equref{ノルム-max}で定義された
    $\norm{\dummy}$はノルムである。
    後述するように，$p$ノルムにおいて$p\to\infty$の極限で再現されることから，これを$\norm{\dummy}_\infty$と書いて$\infty$ノルムとも呼ぶ。
\end{eg}

\begin{defi}[$p$ノルム]\defilabel{ノルム-pノルム}
    $p$を$1$以上の実数とする。
    このとき，$x$の$p$ノルム$\norm{x}_p$を，
    \begin{equation}
...
\begin{prob}\problabel{ノルム-pノルムがノルムであることの証明}
    $p$ノルムがノルムの条件（\defiref{ノルム-定義}）を満たしていることを確かめよ。
\end{prob}

$1$ノルムのことを絶対値ノルムと呼び，絶対値ノルムによって定まる距離をManhattan距離と呼ぶ。
$2$ノルムはEuclidノルムに一致する。

\begin{prop}\proplabel{ノルム-pノルムが一様ノルムに収束する}
    $p\to\infty$の極限で$p$ノルム$\norm{\dummy}_p$は\egref{ノルム-一様}の一様ノルム$\norm{\dummy}_\infty$に一致する。
    \begin{proof}
        ...
        \begin{equation}
            \pare{\sum_{k=0}^{n-1}\abs{x_k}^p}^{\frac{1}{p}}\to \abs{x_M}\quad\pare{p\to\infty}
        \end{equation}
        であり，これは\equref{ノルム-pノルムの極限}の成立を意味している。
    \end{proof}
\end{prop}

以降，有限次元線型空間のノルムは\egref{ノルム-一様}の一様ノルムであるとする。

\begin{nota}\notalabel{ノルム-n-1以下の整数}
...
\begin{prop}\proplabel{ノルム-積分の不等式}
    $I$を$\R$上の閉区間であるとする。
    ...
    \begin{proof}
        ...
        \end{equation}
        である。
        ここで，\equref{ノルム-積分絶対値不等式}の不等式から，
        \begin{equation}
...
\begin{defi}[作用素ノルム]\defilabel{ノルム-作用素ノルム}
...
\begin{prop}\proplabel{ノルム-作用素ノルム具体形}
    \begin{equation}
...
\begin{prop}\proplabel{ノルム-作用素ノルム不等式1}
    \begin{equation}
        \forall x\in \R^n,\ \norm{Ax}\leq \norm{A}\norm{x}
    \end{equation}
    \begin{proof}
...
\begin{prop}\proplabel{ノルム-作用素ノルム三角不等式}
...
\begin{prop}\proplabel{ノルム-作用素ノルムはノルム}
    \defiref{ノルム-作用素ノルム}で定義した$\norm{A}$はノルムである。
    \begin{proof}
        及び，\propref{ノルム-作用素ノルム三角不等式}
        \defiref{ノルム-定義}の4条件を順に確かめる。
        \begin{enumerate}[label=(\roman*)]
            \item （正値性）
            \propref{ノルム-作用素ノルム具体形}から，各項にある絶対値は非負なので，$\norm{A} \geq 0$である。

            \item （一意性）
            \propref{ノルム-作用素ノルム具体形}から，$\norm{A}=0$だとすれば，絶対値の非負性から，どの$j$に関しても$\sum_{k=0}^{n-1}\abs{a_{jk}}=0$であり，これが成立するためには全ての$\pare{j,k}$に関して$A_{jk}=0$でなければならず，このとき$A=O$（零行列）である。
            ...
            \item （三角不等式）
            \propref{ノルム-作用素ノルム三角不等式}から直ちに従う。
        \end{enumerate}
    \end{proof}
\end{prop}
...
\begin{prob}\problabel{ノルム-その1}
    $\R^n$のベクトル$x$のノルムは\egref{ノルム-一様}で述べた一様ノルムとし，
    $\R^n$上の線型作用素$A$のノルムは\defiref{ノルム-作用素ノルム}で与えられた作用素ノルムであるとする。
    $n=3$のとき，
    \begin{equation}
        x=\vctr{-1\\3\\-4},\
        A=\pmat{1 & -1 & 4\\
...
\begin{prob}\problabel{ノルム-その2}
    $\R^n$のベクトルに対して一様ノルムではなく，\egref{ノルム-Euclidノルム}で導入したEuclidノルムを用いた場合，\defiref{ノルム-作用素ノルム}で定義される$\norm{A}$はどのような性質を持つだろうか。
...
\begin{defi}[連続性]\defilabel{ノルム-関数の連続性}
    $f$が$A$上連続であるとは，
    \begin{equation}
...
\begin{prop}\proplabel{ノルム-Lipschitz連続なら一様連続}
...
\begin{prop}\proplabel{ノルム-一様連続なら連続}
    ...
    \begin{proof}
        ...
        \begin{equation}
            ...
            \equlabel{ノルム-恒真命題 exists forall then forall exists}
        \end{equation}
        は恒真である。
        証明木を描いてみると，
        \figref{ノルム-恒真命題 exists forall then forall existsの証明木}に示すように，\equref{ノルム-恒真命題 exists forall then forall exists}が恒真であることが判る。
        \ifdraft{}{
        \begin{figure}[H]
            \centering
            \begin{prooftree}
            ...
            \end{prooftree}
            \caption{\equref{ノルム-恒真命題 exists forall then forall exists}の証明木}
            \figlabel{ノルム-恒真命題 exists forall then forall existsの証明木}
        \end{figure}}
...
\begin{cor}
    ...
    \begin{proof}
        \propref{ノルム-Lipschitz連続なら一様連続}と
        \propref{ノルム-一様連続なら連続}から直ちに従う。
    \end{proof}
\end{cor}
...
\begin{prop}
    ...
    \begin{proof}
        ...
        \end{equation}
        と定めておくと，$I$上で常に$\abs{\fun{f_j}{t}}\leq L$であり，\equref{ノルム-積分絶対値不等式}により，
        \begin{equation}
...
\begin{prop}
    ...
    \begin{proof}
        ...
        \end{equation}
        である。
        $x>y$のときにも平均値の定理により適当な定数$\tilde{c}_j$の存在が言えるので，\equref{ノルム-f'が有界ならLipschitz連続-不等式}は同様に成立する。
        $x=y$のとき，$\norm{0}\leq L\abs{0}$は成立するので\equref{ノルム-f'が有界ならLipschitz連続-不等式}も成立する。
