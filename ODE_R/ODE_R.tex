...
\begin{prop}[定数係数斉次連立1階常微分方程式の解の存在と一意性]\proplabel{ODE_R-定数行列の解の一意性}
    ...
    \begin{proof}
        \eqref{Lipschitz-AxはLipschitz連続}で確かめたように，$f\colon \R\times \R^n\to \R^n$, $\fun{f}{t,x}\coloneqq Ax$は$\R\times \R^n$の任意の有界閉領域上でLipschitz連続である。
...
\begin{prop}[定数係数線型斉次常微分方程式の解の存在と一意性]\proplabel{ODE_R-定数係数線型常微分方程式}
    ...
    \begin{proof}
        ...
        と書き直せる。
        また，
        \equref{ODE_R-定数係数線型常微分方程式-IC}は，
        \begin{equation}
            \fun{q^k}{a}=b^k
        \end{equation}
        となるから，これをまとめて書けば
